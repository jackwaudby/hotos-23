...
public class Main implements Callable<Integer> {
    ...
    public Integer call() throws InterruptedException {
        ...
        createOltpReadClients(URI, personIds, shutdownNotification, m, clients, notifyShutdown, READERS);
        createOltpReadWriteClients(URI, personIds, shutdownNotification, m, clients, notifyShutdown, WRITERS);
        clients.forEach(Thread::start);
        createMammothClient(URI, shutdownNotification, MAMMOTH_DELAY * 1000, Boolean.parseBoolean(BALANCED), m).start();
    ...
    private static void createOltpReadWriteClients(String URI, List<Long> personIds, BlockingQueue<Integer> out, Metrics m, List<Thread> clients, List<BlockingQueue<Integer>> notifyShutdown, int READERS) {
        for (int i = 0; i < READERS; i++) {
            BlockingQueue<Integer> channel = new LinkedBlockingQueue<>();
            notifyShutdown.add(channel);
            var client = createOltpClient(URI, personIds, channel, out, m, true);
            clients.add(client);
        }
    }

    private static void createOltpReadClients(String URI, List<Long> personIds, BlockingQueue<Integer> out, Metrics m, List<Thread> clients, List<BlockingQueue<Integer>> notifyShutdown, int WRITERS) {
        for (int i = 0; i < WRITERS; i++) {
            BlockingQueue<Integer> in = new LinkedBlockingQueue<>();
            notifyShutdown.add(in);
            var oltpClient = createOltpClient(URI, personIds, in, out, m, false);
            clients.add(oltpClient);
        }
    }

    private static Thread createMammothClient(String URI, BlockingQueue<Integer> out, int delayInMillis, boolean balanced, Metrics metrics) {
        return new Thread(() -> {
    ...
    private static Thread createOltpClient(String URI, List<Long> personIds, BlockingQueue<Integer> in, BlockingQueue<Integer> out, Metrics metrics, boolean rw) {
        return new Thread(() -> {
